...
import { BookOpen, Box, Download, Loader2, Sparkles } from "lucide-react"
      ...
      <button type="button" onClick={() => onOpen(card)} className="relative block aspect-[1.18] w-full overflow-hidden rounded-[12px] bg-[#dedfdd] text-left">
        {card.image || card.imageDark
          ? <div className="absolute inset-0">
              {card.image && <img src={card.image} alt={card.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.035] dark:hidden" />}
              {card.imageDark && <img src={card.imageDark} alt={card.title} className="hidden h-full w-full object-cover transition duration-500 group-hover:scale-[1.035] dark:block" />}
            </div>
