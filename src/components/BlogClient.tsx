...
export default function BlogClient() {
  ...
  // Disabled fetching for static build purposes
  // useEffect(() => {
  //   async function fetchBlogs() {
  //     try {
  //       // Get the current host from window object
  ...
  //         const realBlogs = result.data.map((blog: any) => ({
  //           title: blog.title,
  //           slug: blog.slug,
    ...
    <div>
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center py-16">
        <div className="mb-8">
          ...
          <h1 className="font-black text-4xl md:text-5xl mb-4 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
            Research Publications
          </h1>
          
          <div className="max-w-2xl mx-auto">
            <p className="text-xl text-gray-600 mb-2">
              Curating evidence-based insights in <span className="font-semibold text-blue-600">physiotherapy</span> and <span className="font-semibold text-blue-600">rehabilitation medicine</span>
            </p>
      ...
      {/* Render the Blogs component if real data is available */}
      {!usingSampleData && blogs.length > 0 && (
        <div className="py-12">
          <Blogs blogs={blogs} />
        </div>
      )}
